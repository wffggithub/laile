...
class AboutController extends ComController
{
    ...
    public function add()
    {
        ...
        $category = M('category')->field('id,pid,name,link')->where(['pid'=>$cate_id])->order('o asc')->select();
        /*  $tree = new Tree($category);
          $str = "<option value=\$id \$selected>\$spacer\$name</option>"; //生成的形式
          $category = $tree->get_tree(0, $str, 0);*/

       /* foreach ($category as $k=>$v){
            if ($v['link'] != 'About/index') {
                unset($category[$k]);
            }
        }*/
    ...
    public function index($sid = 0, $p = 1)
    {
        ...
        if ($sid) {
            ...
            $where .= "and {$prefix}about.sid in ($sids) ";
        }
        if ($keyword) {
            $where .= "and {$prefix}about.title like '%{$keyword}%' ";
        }
        //默认按照时间降序
        $orderby = "t desc";
        if ($order == "asc") {
       ...
       /* $tree = new Tree($category);
        ...
        $list = $about->field("{$prefix}about.*,{$prefix}category.name")->where($where)->order($orderby)->join("{$prefix}category ON {$prefix}category.id = {$prefix}about.sid")->limit($offset . ',' . $pagesize)->select();
    ...
    public function edit($id)
    {
        ...
        $id = intval($id);
        $about = M('about')->where('id=' . $id)->find();
        if ($about) {
           ...
           /* $tree = new Tree($category);
            $str = "<option value=\$id \$selected>\$spacer\$name</option>"; //生成的形式
            $category = $tree->get_tree(0, $str, $about['sid']);*/
            foreach ($category as $k=>$v){
                if ($v['link'] != 'About/index') {
                    unset($category[$k]);
                }
    ...
    public function update($id = 0)
    {
       ...
       /* if (empty($data['image_path'])) {
        ...
        if ($id) {
            M('about')->data($data)->where('id=' . $id)->save();
           /* addlog('编辑文章，AID：' . $id);*/
            $this->success('恭喜！编辑成功！',U('index'));
        } else {
            $id = M('about')->data($data)->add();
            if ($id) {
                /*addlog('新增文章，AID：' . $aid);*/
