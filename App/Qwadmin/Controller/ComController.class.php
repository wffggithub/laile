...
class ComController extends BaseController
{
    ...
    public function _initialize()
    {
        ...
        $current_action_name = ACTION_NAME == 'del' ? "index" : ACTION_NAME;
        $current = $m->query("SELECT s.id,s.title,s.name,s.tips,s.pid,p.pid as ppid,p.title as ptitle,p.name pname FROM {$prefix}auth_rule s left join {$prefix}auth_rule p on p.id=s.pid where s.name='" . CONTROLLER_NAME . '/' . $current_action_name . "'");
    ...
    /**
     * @param $type 1表示来自Category 2 表示来自Menu 3表示来自 Group 默认0
     * @param $items
     * @param string $id
     * @param string $pid
     * @param string $son
     * @return array
     */
    protected function getMenu($items, $id = 'id', $pid = 'pid', $son = 'children',$type = 0)
    {
        $tree = array();
        $tmpMap = array();
        ...
        foreach ($items as $item) {
            ...
            if ($type != 3) { //2017/10/4  等于3是为了显示所有权限
    ...
    public function check_login(){
       ...
       // $ip = get_client_ip();
        ...
        if ($uid) {
            ...
            if ($user) {
                if ($auth ==  password($uid.$user['user'].$ua.$salt)) {
                    $flag = true;
                    $this->USER = $user;
                }
            }
