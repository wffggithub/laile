...
/*        $category = M('category')->field('id,pid,name')->where(['pid'=>0])->order('o asc')->select();*/
    ...
    public function index($sid = 0, $p = 1)
    {
        ...
        if ($sid) {
            ...
            $where .= "and {$prefix}cooperation.sid in ($sids) ";
        }
        if ($keyword) {
            $where .= "and {$prefix}cooperation.title like '%{$keyword}%' ";
        }
        //默认按照时间降序
        $orderby = "t desc";
        if ($order == "asc") {
        ...
        //$list = $cooperation->field("{$prefix}cooperation.*,{$prefix}category.name")->where($where)->order($orderby)->join("{$prefix}category ON {$prefix}category.id = {$prefix}cooperation.sid")->limit($offset . ',' . $pagesize)->select();
        $list = $cooperation->field("{$prefix}cooperation.*")->where($where)->order($orderby)->limit($offset . ',' . $pagesize)->select();
    ...
    public function del()
    {
        ...
        if ($aids) {
            ...
            if (M('cooperation')->where($map)->delete()) {
    ...
    public function edit($id)
    {
        ...
        $id = intval($id);
        $cooperation = M('cooperation')->where('id=' . $id)->find();
        if ($cooperation) {

            $category = M('category')->field('id,pid,name')->where(['pid'=>0])->order('o asc')->select();
           /* $tree = new Tree($category);
            $str = "<option value=\$id \$selected>\$spacer\$name</option>"; //生成的形式
            $category = $tree->get_tree(0, $str, $cooperation['sid']);*/
            $this->assign('category', $category);//导航
    ...
    public function update($id = 0,$act = null)
    {
        ...
        if ($act == 'order') {
            ...
            $o = I('post.o', 0, 'intval');
            M('cooperation')->data(array('o' => $o))->where("id='{$id}'")->save();
            die('1');
        }
        ...
        $data['banner_image_path'] = I('post.banner_image_path', '', 'strip_tags');
       ...
       /* if (empty($data['image_path'])) {
        ...
        if ($id) {
            M('cooperation')->data($data)->where('id=' . $id)->save();
           /* addlog('编辑文章，AID：' . $id);*/
            $this->success('恭喜！编辑成功！',U('index'));
        } else {
            $id = M('cooperation')->data($data)->add();
            if ($id) {
                /*addlog('新增文章，AID：' . $aid);*/
