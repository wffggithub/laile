...
class LatestController extends ComController
{
    ...
    public function index($sid = 0, $p = 1)
    {
        ...
        if ($sid) {
            ...
            $where .= "and {$prefix}latest.sid in ($sids) ";
        }
        if ($keyword) {
            $where .= "and {$prefix}latest.title like '%{$keyword}%' ";
        }
        //默认按照时间降序
        $orderby = "t desc";
        if ($order == "asc") {
        ...
        $count = $latest->where($where)->count();
        $list = $latest->field("{$prefix}latest.*,{$prefix}category.name")->where($where)->order($orderby)->join("{$prefix}category ON {$prefix}category.id = {$prefix}latest.sid")->limit($offset . ',' . $pagesize)->select();
    ...
    public function edit($id)
    {
        ...
        $id = intval($id);
        $latest = M('latest')->where('id=' . $id)->find();
        if ($latest) {
           ...
           /* $tree = new Tree($category);
            $str = "<option value=\$id \$selected>\$spacer\$name</option>"; //生成的形式
            $category = $tree->get_tree(0, $str, $latest['sid']);*/
            $this->assign('category', $category);//导航
    ...
    public function update($id = 0)
    {
      ...
      /*  if (empty($data['image_path'])) {
        ...
        if ($id) {
            M('latest')->data($data)->where('id=' . $id)->save();
           /* addlog('编辑文章，AID：' . $id);*/
            $this->success('恭喜！编辑成功！',U('index'));
        } else {
            $id = M('latest')->data($data)->add();
            if ($id) {
                /*addlog('新增文章，AID：' . $aid);*/
